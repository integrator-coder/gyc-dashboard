...
async function ensureTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS "ReconSnapshot" (
      id BIGSERIAL PRIMARY KEY,
      "asOf" TIMESTAMPTZ NOT NULL DEFAULT now(),
      payload JSONB NOT NULL
    );
  `)
  await pool.query(`CREATE INDEX IF NOT EXISTS "ReconSnapshot_asOf_idx" ON "ReconSnapshot" ("asOf" DESC);`)
}

async function latest() {
  const { rows } = await pool.query(`SELECT id, "asOf", payload FROM "ReconSnapshot" ORDER BY "asOf" DESC LIMIT 1`)
  return rows[0] || null
}
...
async function fetchBundle(origin) {
  const endpoints = {
    reconDrafts: '/api/recon/drafts',
    missionOverview: '/api/mission-control/overview',
  }
...
export async function GET(req) {
  try {
    ...
    const ins = await pool.query(`INSERT INTO "ReconSnapshot" (payload) VALUES ($1::jsonb) RETURNING id, "asOf"`, [JSON.stringify(payload)])

    return NextResponse.json({ ...payload, snapshot: { source: 'live-refresh', asOf: ins.rows[0].asOf, id: ins.rows[0].id } })
  } catch (error) {
    return NextResponse.json({ error: error.message || 'Failed to build recon snapshot' }, { status: 500 })
  }
}
