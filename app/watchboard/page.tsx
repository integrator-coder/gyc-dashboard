...
// Access control: only Todd, Bruce, Zac
...
export default function WatchBoard() {
  ...
  const [suspicions, setSuspicions] = useState<Suspicion[]>([]);
  ...
  // Fetch data
  useEffect(() => {
    if (status === 'loading' || initializing) return;
    if (!session?.user?.email || !ALLOWED_EMAILS.includes(session.user.email)) return;

    const fetchData = async () => {
      try {
        const [varsRes, companiesRes, suspicionsRes, snapshotRes] = await Promise.all([
          fetch('/api/watchboard/variables'),
          fetch('/api/watchboard/companies'),
          fetch('/api/watchboard/suspicions'),
          fetch('/api/watchboard/snapshot')
        ]);

        const varsData = await varsRes.json();
        const companiesData = await companiesRes.json();
        const suspicionsData = await suspicionsRes.json();
        const snapshotData = await snapshotRes.json();

        if (varsData.success) setVariables(varsData.data);
        if (companiesData.success) setCompanies(companiesData.data);
        if (suspicionsData.success) setSuspicions(suspicionsData.data);
        if (snapshotData.success) setSnapshot(snapshotData.data);
      } catch (error) {
        console.error('WatchBoard data fetch error:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="jarvis-container">
        <div className="jarvis-loading">LOADING MARKET DATA...</div>
        <style jsx>{`
          .jarvis-container {
            min-height: 100vh;
            background: #000308;
