...
function cleanProfile(row) {
  ...
  return {
    ...
    effectiveMrr: toNumber(row.effectiveMrr) || 0,
...
export async function getStripeLinkageReviewQueue({ tenantId = TENANT_ID, acronym = null, includeResolved = false } = {}) {
  ...
  const [profilesRes, stripeRes, linksRes, reviewsRes] = await Promise.all([
    pool.query(
      `SELECT
         ...
         cp.status,
         cp.mrr,
         COALESCE(linked_mrr.total_mrr, cp.mrr, 0) AS "effectiveMrr",
         cp."stripeCustomerId",
         cp."stripeStatus",
         cp."ghlContactId"
       FROM "ClientProfile" cp
       LEFT JOIN (
         SELECT csl."clientProfileId", SUM(sc.mrr)::numeric AS total_mrr
       ...
       ) linked_mrr ON linked_mrr."clientProfileId" = cp.id
  ...
  for (const profile of profiles) {
    ...
    if (currentPrimary || Number(profile.effectiveMrr || 0) !== 0) continue
