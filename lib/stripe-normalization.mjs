...
export async function syncClientStripeLinks(queryable, {
  ...
  if (resolvedPrimaryId) {
    // Clear primary flag for this client's OTHER stripe links
    ...
    // Clear primary flag if this stripe customer is already primary for a DIFFERENT client
    await queryable.query(
      `UPDATE "ClientStripeLink"
       SET "isPrimary" = false,
           "updatedAt" = NOW()
       WHERE "tenantId" = $1
         AND "stripeCustomerId" = $2
         AND "clientProfileId" <> $3
         AND "isPrimary" = true`,
      [tenantId, resolvedPrimaryId, clientProfileId]
    )
  }
