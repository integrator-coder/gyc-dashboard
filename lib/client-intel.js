...
export async function listClientsForUser(user) {
  ...
  if (scope.unrestricted) {
    ...
    // Also pull from Zendesk org tickets — pattern: "ACRONYM - Full Name" or "(ACRONYM)"
    const zdResult = await pool.query(`
      SELECT
    ...
    for (const row of zdResult.rows) {
      const key = normalizeAcronym(row.acronym)
      if (!key || key.length > 8 || clientMap.has(key)) continue // skip if not an acronym-style prefix
      clientMap.set(key, {
        id: key,
        acronym: key,
        name: row.name || key,
