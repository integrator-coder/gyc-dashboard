  ...
  const identityResult = await pool.query(
    `
      ...
      FROM (
        ...
        FROM "ZoomCall" zc
        FULL OUTER JOIN "ClientContract" cc
          ON cc."ghlContactId" = zc."ghlContactId"
          OR upper(COALESCE(cc.acronym, '')) = upper(COALESCE(zc.acronym, ''))
        FULL OUTER JOIN "Client" c
          ON upper(COALESCE(c.acronym, '')) = upper(COALESCE(zc.acronym, cc.acronym, ''))
        WHERE upper(COALESCE(zc.acronym, cc.acronym, c.acronym, '')) = $1
           OR COALESCE(zc."ghlContactId", cc."ghlContactId") IN (
                SELECT "ghlContactId"
                FROM "ClientContract"
...
export async function getClientIntelForUser(user, acronym) {
  ...
  const [ghlContact, contractResult, stripeResult, zoomCallsResult, leadFlowResult, transcriptCountResult] = await Promise.all([
    fetchGhlContact(identity.ghlContactId),
    pool.query(
      `
        SELECT *
        FROM "ClientContract"
    ...
    pool.query(
      `
        WITH months AS (
          ...
          FROM "ClientFunnelMonth" cfm
          WHERE lower(COALESCE(cfm."locationName", '')) LIKE '%igk%'
             OR lower(COALESCE(cfm."locationName", '')) LIKE '%gifted%'
  ...
  const stripe = stripeResult.rows[0] || null
