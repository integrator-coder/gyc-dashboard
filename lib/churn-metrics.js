...
function computeMonthMetrics(subs, month, classifications=[], customerToLogo={}) {
  ...
  const opening=new Map(), openingMonthly=new Map(), openingPif=new Map(), ending=new Map(), retained=new Map(), added=new Set(), lost=new Set(), program=new Set()
  ...
  for(const sub of subs){const amount=calcSubMRR(sub);if(amount<=0)continue;const cid=idOf(sub),logo=logoFor(cid),created=sub.created,canceled=sub.canceled_at
    if(created<start&&(canceled==null||canceled>=start)){add(opening,logo,amount);if(sub.items?.data?.[0]?.price?.recurring?.interval==='year')add(openingPif,logo,amount);else add(openingMonthly,logo,amount);if(!customerToLogo[cid])unmappedOpening.add(cid)}
  ...
  for(const sub of subs){ const amount=calcSubMRR(sub); if(amount<=0)continue; const cid=idOf(sub), logo=logoFor(cid), created=sub.created, canceled=sub.canceled_at
    ...
    if(canceled!=null&&canceled>=start&&canceled<=end){ const row=match(sub), d=classifyCancellation(row)
      ...
      if(opening.has(logo)) lost.add(logo)
    }
  }
  ...
  return {mrr:round(mrr),newMrr:round(newMrr),retainedNewMrr:round(retainedNewMrr),churnedMrr:round(churnedMrr),activeSubscriptions,
    ...
    programChurnMrr:round(programChurnMrr),programChurnClients:program.size,lateralMovementMrr:round(lateralMovementMrr),unmappedOpeningCustomers:[...unmappedOpening],
