...
const {upsertChurnMetrics,migrateChurnClassificationSchema,persistCancellationEvents,loadChurnClassifications}=require('../scripts/monthly-mrr-nrr-update')
...
async function withPostgresSchema(kind,fn){
 ...
 await db.connect();try{await db.query('BEGIN');await db.query(`CREATE SCHEMA "${schema}"`);await db.query(`SET LOCAL search_path TO "${schema}"`)
  await db.query(`CREATE TABLE "ClientProfile" (id bigint primary key, "tenantId" text default 'gyc', acronym text, "companyName" text); CREATE TABLE "ClientStripeLink" ("tenantId" text default 'gyc',"stripeCustomerId" text,"clientProfileId" bigint)`)
