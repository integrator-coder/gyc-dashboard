...
async function fetchGBPLiveData(location) {
  // Use CID if available, otherwise construct keyword
  const keyword = location.cid || location.gbpPlaceId || `${location.locationName} ${location.city} ${location.state}`;
  
  // Build location string from address
  const locationString = `${location.address}, ${location.city}, ${location.state}`;
...
async function main() {
  console.log('🚀 Starting GBP Live Data Sync for ALL locations (v2)...\n');
