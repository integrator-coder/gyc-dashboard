...
3. Run the monthly recompute once. Stripe subscriptions are fetched completely before opening the database transaction. Schema readiness is checked outside it. One `BEGIN`/`COMMIT` boundary then covers audited-event upserts, PIF bridging, derived cancellation events, `MRRHistory`, and every `MonthlyChurnMetrics` month in the batch. Any stable-logo, PIF, taxonomy, reconciliation, history, metric, or injected failure triggers `ROLLBACK`, leaving all prior events and metrics untouched.
