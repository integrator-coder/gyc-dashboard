...
function MovementTable({ title, rows, kind }) {
  ...
  return <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden"><div className="px-5 py-4 border-b border-gray-800"><h3 className="text-white font-semibold">{title}</h3></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="text-gray-400 text-xs uppercase"><tr><th className="text-left px-4 py-3">Client</th><th className="text-left px-4 py-3">Program movement</th><th className="text-right px-4 py-3">Source MRR</th><th className="text-right px-4 py-3">Destination MRR</th><th className="text-right px-4 py-3">Net</th><th className="text-left px-4 py-3">Reason / evidence</th></tr></thead><tbody>{rows.map((r,i)=>{const known=r.destinationMrr!==null&&r.destinationMrr!==undefined;const movement=r.sourceProgram||r.destinationProgram ? `${r.sourceProgram||'Source pending'} → ${r.destinationProgram||'Destination pending'}` : `${r.logoOutcome} / ${r.programOutcome}`;return <tr key={`${r.logoKey}-${i}`} className="border-t border-gray-800 text-gray-200"><td className="px-4 py-3">{r.clientName}</td><td className="px-4 py-3">{kind==='pif' ? `Deferred · ${r.expectedReturnDate ? new Date(r.expectedReturnDate).toLocaleDateString() : 'return date needs review'}` : movement}{kind!=="pif"&&!known&&r.programOutcome==="migrated"&&<span className="block text-xs text-amber-300">Pricing / PIF details pending</span>}</td><td className="px-4 py-3 text-right">{formatCurrency(Number(r.sourceMrr))}</td><td className="px-4 py-3 text-right">{known ? formatCurrency(Number(r.destinationMrr)) : 'Pending evidence'}</td><td className="px-4 py-3 text-right">{known ? formatCurrency(Number(r.destinationMrr)-Number(r.sourceMrr)) : '—'}</td><td className="px-4 py-3">{kind==="pif" && <span className="block text-blue-300 capitalize">{(r.pifLifecycleStatus||"needs_review").replaceAll("_"," ")}</span>}{r.reason || r.reasonCategory || 'Unknown'}<span className="block text-xs text-gray-400">{r.confidence || 'unknown'} confidence{r.evidence ? ` · ${r.evidence}` : ''}</span></td></tr>})}</tbody></table></div></div>
}
      ...
      {selected && (
        <>
          ...
          {leadership && <>
            ...
            <MovementTable title="Internal laterals (including payment details pending)" rows={leadership.laterals} />
